...
session_start();
  require_once "../model/class_model.php";;
	if(ISSET($_POST)){
		...
		$Declined = "cancel";
		$user_ida= $_SESSION['Customer_id'];
	
		$course = $conn->disapprove_events($Declined,$event_id,$user_ida);
		if($course == TRUE){
		    echo '<div class="alert badge bg-danger">Cancelled event Successfully!</div><script> setTimeout(function() {  window.history.go(-0); }, 1000); </script>';
