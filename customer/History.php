...
<head>
	...
	<link rel="shortcut icon" href="img/avatars/20250426_1521_image (1).png" />
...
<!--  -->
		...
		<?php include('includes/header.php'); ?>
			...
			<main class="content">
				<div class="container-fluid p-0">
					...
					<h1 class="h3 mb-3">  <strong>My History</strong></h1>

					<?php
    $conn = new class_model();
	$my_id = $_SESSION['Customer_id'];

    $mybooking = $conn->fetchAll_history($my_id);
    ?>
  ...
  <div class="row">
    ...
    <div class="col-lg-3">
      <select id="sortStatus" class="form-select ms-auto">
        <option value="all" selected>Status sort</option>
						...
						foreach ($mybooking as $row) { ?>
							<tr>
								...
								<td>
										...
										<?php }elseif($row['Event_status']== "Declined"){ ?>
											<span class="badge bg-danger">
											Declined
											</span>
											<?php }elseif($row['Event_status']== "Payable"){ ?>
												<span class="badge bg-primary">
											Payable
											</span>
											
											<?php }elseif($row['Event_status']== "Expired"){ ?>
												<span class="badge bg-primary">
											Expired
											</span>
											<?php }else{ ?>
...
<?php  } ?>
								...
								<td>
										<?php $ids=$row['Cid'];
										$Cname = $conn->fetchAll_customer($ids);
										foreach($Cname as $srow){
									...
									<td>
										...
										$Ccname = $conn->fetchAll_Us($Uids);
										foreach($Ccname as $row){

											echo $row['Fullname']; echo "("  .$row['role'].")";
										}
				...
				<div class="container-fluid">
					<div class="row text-muted">
						<div class="col-6 text-start">
							<p class="mb-0">
								<a class="text-muted" href="" target="_blank"><strong> FUNCTION HALL RESERVATION AND BILLING SYSTEM </strong></a> 
							</p>
						</div>
...
$(document).ready(function(){
  ...
  $("#sortStatus").on("change", function() {
    ...
    $("table tbody tr").each(function() {
      var statusText = $(this).find("td").eq(3).text().toLowerCase(); // 4th column: Status
      if(selected === "all") {
        $(this).show();
      } else {
        $(this).toggle(statusText.indexOf(selected) !== -1);
      }
    });
  });

  // (Optional) Keep your search filter as is
